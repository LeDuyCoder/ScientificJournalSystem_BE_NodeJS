...
export const getAllArticles = async (firstParam = {}, offsetParam = 0, sortByParam = 'created_at', sortOrderParam = 'DESC') => {
    try {
        ...
        if (search && search.trim()) {
            ...
            values.push(rawSearch); // $1: raw search term for relevance
            values.push(`%${rawSearch}%`); // $2: pattern for WHERE ILIKE
            where.push(`(a."title" ILIKE $2 OR a."doi" ILIKE $2 OR a."abstract" ILIKE $2)`);
        }
        ...
        const query = `
            ...
            ORDER BY 
                ${search ? `CASE 
                    ...
                    WHEN a."title" ILIKE '%' || $1 || '%' THEN 3
                    ELSE 4
                END ASC,` : ''}
                ${column} ${order} NULLS LAST, a."article_id" DESC
