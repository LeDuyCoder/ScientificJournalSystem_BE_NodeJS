...
import {
  createSubTopicArticleRelationships,
  topicExists,
  updateSubTopicArticleRelationships,
  ...
  try {
    ...
    if (article.is_deleted === true) {
      return res.status(410).json({
        // Hoặc dùng 404 tùy cấu trúc dự án của bạn
        success: false,
...
export const createArticle = async (req, res) => {
  ...
  if (publication_year === undefined || publication_year === null) {
    return res
      .status(400)
      .json({ success: false, message: "Publication year is required" });
  }
  if (typeof publication_year !== "number") {
    return res
      .status(400)
      .json({ success: false, message: "Publication year must be a number" });
  }
  if (
    issue_id !== undefined &&
    issue_id !== null &&
    typeof issue_id !== "number"
  ) {
    return res
      .status(400)
      .json({ success: false, message: "Issue ID must be a number" });
  }
  if (
    ...
    return res
      .status(400)
      .json({ success: false, message: "Primary topic must be a number" });
  }
  ...
  if (Array.isArray(authors) && !authors.every((id) => Number.isInteger(id))) {
    return res
      .status(400)
      .json({ success: false, message: "Each author ID must be an integer" });
  }
    ...
    if (Array.isArray(keywords)) {
      ...
      if (invalidKeyword) {
        return res.status(400).json({
          success: false,
          message:
            "Keywords must be an object mapping string keyword names to numeric scores",
        });
      }
    } else {
      return res.status(400).json({
  ...
  if (
    Array.isArray(sub_topic) &&
    !sub_topic.every(
      (item) => typeof item === "string" || Number.isInteger(item),
    )
  ) {
    return res.status(400).json({
      success: false,
...
export const updateArticle = async (req, res) => {
  ...
  try {
    ...
    if (!article) {
      return res
        .status(404)
        .json({ success: false, message: "Article không tìm thấy" });
    }
    ...
    if (dataBody.keywords !== undefined) {
      if (Array.isArray(dataBody.keywords)) {
        ...
        dataBody.keywords !== null &&
        typeof dataBody.keywords === "object"
      ) {
